...
// Instancia global o privada de la sesión para registrar la masa semántica por posición
static nai::MapaMasaSemantica g_nai_mapa_masa;
...
float * llama_context::get_embeddings_layer_inp_ith(uint32_t lid, int32_t i) {
    output_reorder();

    try {
        if (lid >= embd_layer_inp.size() || !embd_layer_inp[lid].has_data()) {
            throw std::runtime_error("no embeddings for layer");
        }

        const int64_t j = output_resolve_row(i);
        const uint32_t n_embd = model.hparams.n_embd;
        return embd_layer_inp[lid].data + j * n_embd;
    } catch (const std::exception & err) {
        LLAMA_LOG_ERROR("%s: invalid layer input embeddings id %d for layer %u, reason: %s\n", __func__, i, lid, err.what());
        #ifndef NDEBUG
                GGML_ABORT("fatal error");
        #else
                return nullptr;
...
void llama_context::output_reorder() {
    ...
    for (size_t s = 0; s < output_swaps.size(); ++s) {
        ...
        if (!sampling.samplers.empty()) {
            GGML_ASSERT(sampling.logits.size > 0 && "sampling.logits buffer no inicializado");
            GGML_ASSERT(sampling.probs.size > 0 && "sampling.probs buffer no inicializado");
            GGML_ASSERT(sampling.candidates.size > 0 && "sampling.candidates buffer no inicializado");

            // Verificación de seguridad antes de acceder a los arreglos
            if (sampling.logits.data != nullptr && 
                sampling.probs.data != nullptr && 
                sampling.candidates.data != nullptr &&
                sampling.sampled.data != nullptr) {
...
ggml_status llama_context::graph_compute(
    ...
    // =========================================================================
    // --- [NAI HOOK]: REGISTRO DE MASA SEMÁNTICA POST-CÓMPUTO ---
    // =========================================================================
    // Sincronizamos brevemente los logits resultantes del grafo para registrar la masa
    if (this->logits.data != nullptr) {
        float * logits_ptr = this->get_logits_ith(-1);
        if (logits_ptr) {
            InerciaLatenteResult res = nai::evaluar_inercia_tensor(logits_ptr, this->model.vocab.n_tokens());
            
            // Registramos la masa semántica basada en la entropía/varianza del estado
            // Usamos n_bytes o el contador de secuencia para mapear la posición actual
            int pos_actual = this->n_outputs;
            g_nai_mapa_masa.registrar_masa(pos_actual, res.entropia);
        }
    }
    // =========================================================================
...
public:
    llama_io_write_host(uint8_t * p, size_t len) 
        : ptr(p), buf_size(len) {}
    ...
    ~llama_io_write_host() noexcept override {
        try {
            flush();
        } catch (...) {
        }
...
public:
    ...
    ~llama_io_read_host() noexcept override {
        try {
            flush();
        } catch (...) {
            // Previene crashes si falla el backend durante la destrucción
        }
    }
...
void llama_context::state_write(llama_io_write_i & writer, const llama_token * tokens, size_t n_tokens) const{
    const uint32_t magic = LLAMA_SESSION_MAGIC;
    const uint32_t version = LLAMA_SESSION_VERSION;

    writer.write(&magic, sizeof(magic));
    writer.write(&version, sizeof(version));

    const uint32_t n_tok = (uint32_t) n_tokens;
    writer.write(&n_tok, sizeof(n_tok));
    if (n_tok > 0 && tokens != nullptr) {
        writer.write(tokens, n_tok * sizeof(llama_token));
    }

    if (memory) {
        memory->state_write(writer);
    }
}

bool llama_context::state_read(llama_io_read_i & reader, llama_token * tokens_out, size_t n_tokens_max, size_t * n_tokens_out) {
    uint32_t magic = 0;
    uint32_t version = 0;
    ...
    if (magic != LLAMA_SESSION_MAGIC || version != LLAMA_SESSION_VERSION) {
        LLAMA_LOG_ERROR("%s: invalid session magic or version\n", __func__);
        return false;
    }

    uint32_t n_tok = 0;
    reader.read(&n_tok, sizeof(n_tok));

    if (tokens_out != nullptr && n_tokens_max >= n_tok) {
        reader.read(tokens_out, n_tok * sizeof(llama_token));
        if (n_tokens_out) {
            *n_tokens_out = n_tok;
        }
    } else if (n_tok > 0) {
        reader.read(nullptr, n_tok * sizeof(llama_token));
        if (n_tokens_out) {
            *n_tokens_out = 0;
        }
    }
    ...
    if (memory.get() != nullptr) {
        try {
            memory->state_read(reader);
        } catch (...) {
            LLAMA_LOG_ERROR("%s: failed to restore memory state\n", __func__);
            return false;
        }
    }
...
// Helper para lectura de memoria en el formato de llama.cpp
struct llama_io_buffer_reader : public llama_io_read_i {
    const uint8_t * src;
    size_t size;
    size_t read_bytes = 0;

    llama_io_buffer_reader(const uint8_t * src, size_t size) : src(src), size(size) {}

    void read(void * dst, size_t n) override {
        if (dst && read_bytes + n <= size) {
            memcpy(dst, src + read_bytes, n);
        }
        read_bytes += n;
    }

    void read_tensor(ggml_tensor * tensor, size_t offset, size_t n) override {
        if (tensor && tensor->data) {
            read((uint8_t *)tensor->data + offset, n);
        }
    }
...
size_t llama_context::state_get_size() {
    llama_io_write_dummy writer(false);
    state_write(writer, nullptr, 0);
    return writer.n_bytes();
}

// 1. PRIMERO: Definir la struct llama_io_write_buffer
struct llama_io_write_buffer : public llama_io_write_i {
    uint8_t * dst;
    size_t size;
    size_t written = 0;

    llama_io_write_buffer(uint8_t * dst, size_t size) : dst(dst), size(size) {}

    void write(const void * src, size_t len) override {
        if (dst && written + len <= size) {
            memcpy(dst + written, src, len);
        }
        written += len;
    }

    // Firma con los 3 parámetros equivalentes a la interfaz de escritura:
    void write_tensor(ggml_tensor * tensor, size_t offset, size_t n) override {
        if (tensor && tensor->data) {
            write((const uint8_t *)tensor->data + offset, n);
        }
    }
...
// 2. SEGUNDO: Usar la struct en state_get_data
size_t llama_context::state_get_data(uint8_t * dst, size_t size) {
    llama_io_write_buffer writer(dst, size);
    state_write(writer, nullptr, 0);
    return writer.n_bytes();
}
...
size_t llama_context::state_set_data(const uint8_t * src, size_t size) {
    ...
    llama_io_buffer_reader reader(src, size);
    uint32_t magic = 0;
    uint32_t version = 0;
    ...
    if (magic != LLAMA_SESSION_MAGIC) {
        LLAMA_LOG_ERROR("%s: formato de estado invalido (magic mismatch: 0x%08x vs 0x%08x)\n", 
                        __func__, magic, LLAMA_SESSION_MAGIC);
        return 0;
    }

    if (version != LLAMA_SESSION_VERSION) {
        LLAMA_LOG_ERROR("%s: version de sesion no compatible (version: %u, esperada: %u)\n", 
                        __func__, version, LLAMA_SESSION_VERSION);
        return 0;
    }
    ...
    if (n_tok > 0) {
        std::vector<llama_token> loaded_tokens(n_tok);
        reader.read(loaded_tokens.data(), n_tok * sizeof(llama_token));
...
bool llama_context::state_load_file(const char * filepath, llama_token * tokens_out, size_t n_tokens_max, size_t * n_tokens_out) {
    llama_file file(filepath, "rb");
    llama_io_read_file reader(&file);
    
    return state_read(reader, tokens_out, n_tokens_max, n_tokens_out);
}

bool llama_context::state_save_file(const char * filepath, const llama_token * tokens, size_t n_tokens)  {
    llama_file file(filepath, "wb");
    llama_io_write_file writer(&file);
    
    state_write(writer, tokens, n_tokens);
    
    return writer.n_bytes() > 0;
}
...
float * llama_get_embeddings_layer_inp_ith(llama_context * ctx, uint32_t lid, int32_t i) {
    ctx->synchronize();

    return ctx->get_embeddings_layer_inp_ith(lid, i);
}
